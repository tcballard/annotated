...
test('the checksummed packaged extension completes the Gate B browser loop', async ({}, testInfo) => {
  ...
  try {
    ...
    // A row about the page you are on acts on that page: the primary action
    // highlights the exact passage in place instead of opening a duplicate
    // tab. The W3C text-fragment deep link stays on the row's href for every
    // reader who is anywhere else.
    const articleOpenAction = panel.locator('.post .act.primary').first();
    expect(await articleOpenAction.getAttribute('href')).toContain(`${articleSourceUrl}#:~:text=`);
    expect(await articleOpenAction.getAttribute('title')).toContain('Show on this page');
    const pagesBeforeArticleOpen = context.pages().length;
    await articleOpenAction.click();
    await expect.poll(() => contentPage.evaluate(() => (
      Boolean(window.CSS?.highlights && CSS.highlights.has('annotated-passage'))
      || String(window.getSelection() || '').length > 0
    )), { timeout: 10_000 }).toBe(true);
    expect(context.pages().length).toBe(pagesBeforeArticleOpen);
    ...
    // The published clip is about the player page the panel is bound to, so
    // its primary action plays the exact clip on the page's own player —
    // seek to the in-mark, no duplicate tab. The timestamped deep link stays
    // on the href for readers elsewhere.
    const videoOpenAction = panel.locator('.post .act.primary').first();
    expect(await videoOpenAction.getAttribute('href')).toMatch(new RegExp(`^${playerSourceUrl.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}#t=1[12]$`));
    expect(await videoOpenAction.getAttribute('title')).toContain('Show on this page');
    const pagesBeforeVideoOpen = context.pages().length;
    await videoOpenAction.click();
    await expect.poll(() => contentPage.evaluate(() => document.querySelector('video')?.currentTime || 0), { timeout: 10_000 }).toBeGreaterThanOrEqual(10);
    expect(context.pages().length).toBe(pagesBeforeVideoOpen);
    await contentPage.bringToFront();
