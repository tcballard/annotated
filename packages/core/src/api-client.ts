...
export const createApiClient = ({ origin = '', fetchFn }: ApiClientOptions = {}) => {
  ...
  return {
    ...
    savePreferences: (preferences: unknown) => apiRequest('/api/preferences', { method: 'PUT', body: JSON.stringify({ preferences }) }),
    ...
    sourceGraph: (sourceId: string, cursor?: string) => apiRequest(`/api/sources/exact/${encodeURIComponent(sourceId)}${cursor ? `?cursor=${encodeURIComponent(cursor)}` : ''}`),
    share: (slug: string) => apiRequest(`/api/annotations/${encodeURIComponent(slug)}/share`),
    event: (payload: Record<string, any>) => apiRequest('/api/events', { method: 'POST', body: JSON.stringify(payload) }),
    createPublisherChallenge: (domain: string) => apiRequest('/api/publishers/challenges', { method: 'POST', body: JSON.stringify({ domain }) }),
    verifyPublisherChallenge: (id: string) => apiRequest(`/api/publishers/challenges/${encodeURIComponent(id)}/verify`, { method: 'POST', body: '{}' }),
    publisherWorkspace: (id: string, cursor?: string) => apiRequest(`/api/publishers/workspaces/${encodeURIComponent(id)}${cursor ? `?cursor=${encodeURIComponent(cursor)}` : ''}`),
    publisherReply: (workspaceId: string, annotationId: string, body: string) => apiRequest(`/api/publishers/workspaces/${encodeURIComponent(workspaceId)}/annotations/${encodeURIComponent(annotationId)}/reply`, { method: 'POST', body: JSON.stringify({ body }) }),
    publisherClaimsBulk: (workspaceId: string, payload: Record<string, any>) => apiRequest(`/api/publishers/workspaces/${encodeURIComponent(workspaceId)}/claims/bulk`, { method: 'POST', body: JSON.stringify(payload) }),
