...
export const signIn = async (provider) => {
  ...
  if (!['google', 'x'].includes(provider)) throw new Error('That sign-in provider is not supported.');
  ...
  let callbackUrl;
  try { callbackUrl = new URL(callback); } catch { throw new Error('The sign-in callback was not a valid extension URL.'); }
  const redirectUrl = new URL(returnTo);
  if (callbackUrl.origin !== redirectUrl.origin || callbackUrl.pathname !== redirectUrl.pathname) throw new Error('The sign-in callback did not return to this extension.');
