// Deep links back to the original source (panel copy of src/deep-link.js):
// timestamped URLs for media, W3C text fragments for article passages.
...
export const textFragment = (excerpt, { prefix = '', suffix = '' } = {}) => {
  ...
  if (text.length <= 150) {
    ...
    const head = [];
    while (head.join(' ').length < 40 && words.length) head.push(words.shift());
    const tail = [];
    while (tail.join(' ').length < 40 && words.length) tail.unshift(words.pop());
