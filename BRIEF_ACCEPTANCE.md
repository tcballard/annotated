...
| --- | --- | --- | --- |
| Chrome side-panel extension is the primary capture surface | partial | Manifest V3 side panel reads the active tab and selected text, reloads on tab activation/navigation, uses an HTTPS-required deployed API origin, expires session tokens, exchanges OAuth tickets for bearer sessions, records bounded audio in IndexedDB, uploads server-side media, and queues retryable publishes in bounded local metadata; bounded client request IDs prevent duplicate publishes, uploaded audio IDs survive annotation retries, and repeated/non-retryable failures become bounded blocked items. The extension now declares Chrome 114+, handles service-worker setup/open/retry failures, validates stored API origins on read, has a runtime-only packaging script plus a Chrome Web Store readiness record, and uses an explicit `[hidden]` state contract, inline SVG icon language, accessible pressed/status state, and reduced-motion handling. An isolated Chrome DevTools MCP run loaded v0.1.0, opened the live side-panel page, triggered the action against `https://example.com`, and found no page or service-worker console errors. | Docked packaged-browser acceptance, microphone/offline/service-worker recovery, real deployed-origin run, and store assets/privacy-policy/contact details. |
