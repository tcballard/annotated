...
const numericSeconds = (value: unknown): number => {
  const number = Number(value);
  return Number.isFinite(number) ? Math.round(number) : 0;
};

const clampSeconds = (value: unknown, max: number): number => Math.max(0, Math.min(max, numericSeconds(value)));
...
export const normalizeClipRange = (
  ...
  { max = MAX_CLIP_SECONDS, maxDuration = MAX_CLIP_SECONDS, min = MIN_CLIP_SECONDS, allowEmpty = false }: ClipRangeOptions = {},
): ClipRange => {
