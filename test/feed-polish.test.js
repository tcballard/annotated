...
test('on phones the feed rail stays a top rail and swipes flick panes', () => {
  // The bottom dock is gone: the native app switches feeds from the top
  // rail, and the two phone experiences read as one. Swiping between
  // panes stays — same behavior as the app's pager.
  assert.doesNotMatch(css, /\.feedhead \{ position: fixed/, 'the rail never docks to the bottom');
