...
test('production PostgreSQL and S3 adapters work against real services', {
  ...
  const records = {
    ...
    media: { id: `${fixturePrefix}-media`, annotationId, ownerId: markerId, key: `${fixturePrefix}/source.mp4`, mimeType: 'video/mp4', bytes: 64, status: 'ready' },
    mediaJobs: { id: `${fixturePrefix}-job`, annotationId, ownerId: markerId, sourceUrl: 'https://example.com/source.mp4', mediaUrl: 'https://example.com/source.mp4', provider: 'direct', sourceType: 'video', clipStart: 0, clipEnd: 10, status: 'processing', attempts: 0, workerId: `${fixturePrefix}-worker`, traceId: `${fixturePrefix}-trace`, leaseUntil: new Date(Date.now() + 60_000).toISOString(), createdAt: new Date().toISOString() },
