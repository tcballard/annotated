...
For a production-shaped run, copy `.env.example`, set `ANNOTATED_STORAGE=postgres` and `ANNOTATED_ASSET_STORAGE=s3`, provide `DATABASE_URL`, the S3 credentials, and Google OAuth credentials, set `OAUTH_PROVIDERS=google`, run `npm run db:migrate`, and then start the server. Production refuses to start without durable storage, the enabled provider credentials, and the app origin. Add X later by setting `OAUTH_PROVIDERS=google,x` and supplying its client and redirect configuration; local file storage and the local account remain explicit development adapters.
...
Source processing uses bounded HTML metadata extraction with entity-decoded titles, descriptions, excerpts, media URLs, and canonical links, safe three-hop redirect handling, DNS-rebinding checks at every fetched hop, and canonical-link preservation. Podcast host URLs and RSS/Atom feed URLs use bounded first-episode parsing for title, author, description, artwork, canonical show links, and safe enclosure URLs. Extensionless or signed podcast enclosures are handed directly to FFmpeg after the same public-URL validation; other provider streams use the configured `yt-dlp` adapter. Media is transcoded asynchronously; provider-returned stream URLs are revalidated before FFmpeg runs, FFprobe inspects duration, audio streams, and video height before a clip is marked ready, and ready public/feed annotations use native browser video/audio controls instead of prototype-only play buttons. Active provider/FFmpeg processes can be cancelled by the owner, late completions cannot revive a cancelled job, and objects uploaded by an abandoned job are deleted. Set `YTDLP_BIN`, `MEDIA_WORKER_MAX_ATTEMPTS`, and `MEDIA_WORKER_RETRY_DELAY_MS` explicitly for a deployed worker. A provider binary or credential is not assumed merely because the adapter exists.
