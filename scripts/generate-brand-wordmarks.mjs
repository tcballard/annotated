...
const inverseSvg = file('logo-inverse', 'The Annotated wordmark in white with a terracotta full stop, outlined to paths.', inversePaths, inkWidth);
await writeFile(new URL('../public/brand/logo-inverse.svg', import.meta.url), inverseSvg);

// The share card draws the same lockup. Satori composes the card server-side
// from whatever fonts the container carries (DejaVu, in production) — so the
// wordmark rides along as the finished drawing instead of being retyped.
await writeFile(
  new URL('../server/og-wordmark.js', import.meta.url),
  `// GENERATED by scripts/generate-brand-wordmarks.mjs — do not edit.
// The outlined inverse lockup for the share card's chrome bar, so the
// card never draws the brand in the container's fonts.
export const OG_WORDMARK = {
  width: ${inkWidth},
  height: ${BOX_HEIGHT},
  svg: ${JSON.stringify(inverseSvg.trim())},
};
`,
);

console.log(`wordmarks outlined: viewBox 0 0 ${inkWidth} ${BOX_HEIGHT} (was 0 0 88 24, ink needed ~102); og module refreshed`);
