...
- The extension's bearer session token is ephemeral `chrome.storage.session` state; pending capture payloads are bounded metadata in `chrome.storage.local` and are retried by the service worker.
